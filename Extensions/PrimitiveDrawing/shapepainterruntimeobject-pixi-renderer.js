...
(function(gdjs2) {
  ...
  class ShapePainterRuntimeObjectPixiRenderer {
    ...
    drawLine(x1, y1, x2, y2, thickness) {
      ...
      if (y2 === y1) {
        ...
        this._graphics.drawPolygon(x1 + xIncrement, y1 - yIncrement, x1 - xIncrement, y1 + yIncrement, x2 - xIncrement, y2 + yIncrement, x2 + xIncrement, y2 - yIncrement);
      }
      this._graphics.endFill();
    }
    ...
    drawStar(x1, y1, points, radius, innerRadius, rotation) {
      this.updateOutline();
      this._graphics.beginFill(this._object._fillColor, this._object._fillOpacity / 255);
      this._graphics.drawStar(x1, y1, points, radius, innerRadius ? innerRadius : radius / 2, rotation ? gdjs2.toRad(rotation) : 0);
      this._graphics.closePath();
      this._graphics.endFill();
    }
    drawArc(x1, y1, radius, startAngle, endAngle, anticlockwise, closePath) {
      this.updateOutline();
      this._graphics.beginFill(this._object._fillColor, this._object._fillOpacity / 255);
      this._graphics.moveTo(x1 + radius * Math.cos(gdjs2.toRad(startAngle)), y1 + radius * Math.sin(gdjs2.toRad(startAngle)));
      this._graphics.arc(x1, y1, radius, gdjs2.toRad(startAngle), gdjs2.toRad(endAngle), anticlockwise ? true : false);
      if (closePath) {
        this._graphics.closePath();
      }
      this._graphics.endFill();
    ...
    updateOutline() {
      this._graphics.lineStyle(this._object._outlineSize, this._object._outlineColor, this._object._outlineOpacity / 255);
    }
    updateXPosition() {
  ...
  gdjs2.ShapePainterRuntimeObjectRenderer = ShapePainterRuntimeObjectPixiRenderer;
})(gdjs || (gdjs = {}));
